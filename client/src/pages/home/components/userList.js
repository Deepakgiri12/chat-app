...
function UsersList({searchKey, socket, onlineUser}){
    ...
    console.log("1. startNewChat called");

    let response = null;

    try {
        console.log("2. Before showLoader");
        dispatch(showLoader());
    ...
    useEffect(() => {
        socket.off('set-message-count').on('set-message-count', (message) => {
            ...
            //3. CREATE A NEW ARRAY LATEST CHAT ON TOP & THAN OTHER CHATS
                    ...
                    <div className="filter-user-display">
                        ...
                        { !allChats.find(chat => chat.members.map(m => m._id).includes(user._id)) &&
                            <div className="user-start-chat">
                                <button className="user-start-chat-btn" onClick={() =>  {startNewChat(user._id);  console.log("Button clicked");}}>
                                    Start Chat
                                </button>
                            </div>
                        }
